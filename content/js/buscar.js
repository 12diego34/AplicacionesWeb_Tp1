jQuery(document).ready(function (){
    ...
    function armar_modal(id, callback){
        ...
        .done(function (data) {
            jQuery('.modal-title-comparacion').html(data.original_title);
            var markup_pelicula =
            ...
            jQuery('#btn-comparar').click(function (){
                var select = jQuery('#select-grupo-comparacion');
                consultar_puntaje_grupos(jQuery(select)[0].selectedIndex+1, id);
                var strGrupo = jQuery(select)[0].options[jQuery(select)[0].selectedIndex].text;
            ...
            jQuery.ajax({
                ...
                .done(function( data_api ) {
                    ...
                    console.log(rating);
                    jQuery('#rating-'+id).rating('rate', rating );
                    jQuery('#rating-local').html(rating);
            });
            callback();
        })
    }
    ...
    function consultar_puntaje_grupos (grupo, id) {
        ...
        jQuery.ajax({
            ...
            if (!jQuery.isEmptyObject(data)){
                ...
                jQuery('#rating-externo').html('No existe la pelicula seleccionada');
                jQuery('#rating-externo').addClass("rating-err");
            }
        }).fail(function (data) {
            jQuery('#rating-local').addClass("rating-win");
            jQuery('#rating-externo').html("Error en la conexi&oacute;n");
            jQuery('#rating-externo').addClass("rating-err");
